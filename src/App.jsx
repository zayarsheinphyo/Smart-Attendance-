...
export default function App() {
  ...
  const getStats = (period) => {
    ...
    const isOfficeActiveDay = (dNum, mNum, yNum) => {
        if (dNum === todayD && parseInt(mNum,10) === todayM && parseInt(yNum,10) === todayY) return true;
        const cacheKey = `${dNum}-${mNum}-${yNum}`;
        if (activeDaysCache[cacheKey] !== undefined) return activeDaysCache[cacheKey];
        const isActive = records.some(r => isSameDate(r.date, dNum, mNum, yNum));
         ...
         for (let d = 1; d <= limitDay; d++) {
             ...
             if (dayRecords.length > 0) {
                 ...
                 if (isCurrentMonthView && d === limitDay) {
                     ...
                     if (isOfficeActiveDay(d, sMonth, sYear)) {
