...
export default function App() {
  ...
  // --- Auto Generate Absent Records ---
  useEffect(() => {
      if (!user || employees.length === 0) return;
      
      const timer = setTimeout(async () => {
          if (window.__absences_generated) return;
          window.__absences_generated = true;
          ...
          for (const emp of employees) {
              let startDay = 1;
              if (emp.createdAt) {
                  const cDate = new Date(emp.createdAt);
                  if (!isNaN(cDate.getTime())) {
                      // Use exact employee creation date with 0 leniency.
                      if (cDate.getFullYear() === currentY && (cDate.getMonth() + 1) === currentM) {
                          startDay = Math.max(1, cDate.getDate());
                      } else if (cDate.getFullYear() > currentY || (cDate.getFullYear() === currentY && (cDate.getMonth() + 1) > currentM)) {
                          continue; 
                      }
                  }
              }

              for (let d = startDay; d <= todayD; d++) {
                  if (emp.offDays?.includes(d)) continue;
                  
                  const isPastDay = d < todayD;
                  const isTodayPastNoon = d === todayD && isPastNoon;

                  if (isPastDay || isTodayPastNoon) {
                      const officeActiveThatDay = records.some(r => {
                          if (!r.date) return false;
                          const p = r.date.split('/');
                          if (p.length !== 3) return false;
                          return parseInt(p[0], 10) === d && parseInt(p[1], 10) === currentM && parseInt(p[2], 10) === currentY && !r.isAutoGenerated && !r.isAbsent;
                      });

                      if (!officeActiveThatDay) continue;
                      ...
                      const hasRecord = records.some(r => {
                          ...
                          const p = r.date.split('/');
                          if (p.length !== 3) return false;
                          return parseInt(p[0], 10) === d && parseInt(p[1], 10) === currentM && parseInt(p[2], 10) === currentY;
                      });

                      if (!hasRecord) {
                          const dateStr = `${String(d).padStart(2, '0')}/${String(currentM).padStart(2, '0')}/${currentY}`;
                          try {
                              await addDoc(collection(db, 'artifacts', appId, 'users', "shared_company", 'records'), {
                                  ...
                                  photo: `https://ui-avatars.com/api/?name=${encodeURIComponent(emp.name)}&background=f43f5e&color=fff`,
                                  createdAt: Date.now()
                              });
                              records.push({ employeeId: emp.id, date: dateStr, isAbsent: true });
                          } catch (e) {
                              console.error(e);
                          }
                      }
  ...
  const openDetailModal = (emp) => {
      ...
      const d = new Date();
      setDetailModalDate(`${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}-${String(d.getDate()).padStart(2, '0')}`);
  };

  const renderDetailModalStatus = () => {
      if (!detailModalDate || !detailModalEmp) return null;
      const [y, m, d] = detailModalDate.split('-');
      if (!y || !m || !d) return null;
      const formattedDate = `${String(parseInt(d, 10)).padStart(2, '0')}/${String(parseInt(m, 10)).padStart(2, '0')}/${y}`;
      const dayNum = parseInt(d, 10);
      const isOffDay = detailModalEmp.offDays?.includes(dayNum);
      
      const dayRecords = records.filter(r => r.employeeId === detailModalEmp.id && r.date === formattedDate);
      
      if (dayRecords.length > 0) {
          const absentRecord = dayRecords.find(r => r.isAbsent);
          if (absentRecord) {
              return (
                  <div className="flex flex-col items-center gap-2">
                      <XCircle className="w-8 h-8 text-red-500" />
                      <span className="text-red-600 font-bold">ပျက်ကွက်</span>
                  </div>
              );
          }
          const lateRecord = dayRecords.find(r => r.isLate);
          if (lateRecord) {
              return (
                  <div className="flex flex-col items-center gap-2">
                      <AlertCircle className="w-8 h-8 text-orange-500" />
                      <span className="text-orange-600 font-bold">နောက်ကျ ({lateRecord.lateMinutes} မိနစ်)</span>
                      <span className="text-xs text-slate-500">ဝင်ရောက်သည့်အချိန်: {lateRecord.time}</span>
                  </div>
              );
          }
          return (
              <div className="flex flex-col items-center gap-2">
                  ...
                  <span className="text-green-600 font-bold">အချိန်မှန်</span>
                  <span className="text-xs text-slate-500">ဝင်ရောက်သည့်အချိန်: {dayRecords[0].time}</span>
              </div>
          );
      }
      ...
      if (isOffDay) {
          return (
              <div className="flex flex-col items-center gap-2">
                  ...
                  <span className="text-indigo-600 font-bold">နားရက်</span>
              </div>
          );
      }
      ...
      if (targetDate > today) {
          return <div className="text-center text-slate-500 font-medium py-2">မှတ်တမ်း မရှိသေးပါ</div>;
      }
      
      const officeActive = records.some(r => r.date === formattedDate && !r.isAutoGenerated && !r.isAbsent);
      if (officeActive) {
          if (targetDate.getTime() === today.getTime()) {
              const now = new Date();
              if (now.getHours() >= 12) {
                  return <div className="flex flex-col items-center gap-2"><XCircle className="w-8 h-8 text-red-500" /><span className="text-red-600 font-bold">ပျက်ကွက်</span></div>;
              } else {
                  return <div className="text-center text-slate-500 font-medium py-2">စာရင်းမသွင်းရသေးပါ</div>;
              }
          }
          let skipBecauseNotHired = false;
          if (detailModalEmp.createdAt) {
              const empCreatedAtStartOfDay = new Date(detailModalEmp.createdAt);
              if (!isNaN(empCreatedAtStartOfDay.getTime())) {
                  empCreatedAtStartOfDay.setHours(0,0,0,0);
                  if (targetDate < empCreatedAtStartOfDay) {
                      skipBecauseNotHired = true;
                  }
              }
          }
          if (skipBecauseNotHired) {
               return <div className="text-center text-slate-500 font-medium py-2">အလုပ်မဝင်သေးပါ</div>;
          }
          return <div className="flex flex-col items-center gap-2"><XCircle className="w-8 h-8 text-red-500" /><span className="text-red-600 font-bold">ပျက်ကွက်</span></div>;
      } else {
          return <div className="text-center text-slate-500 font-medium py-2">ရုံးပိတ်ရက် / မှတ်တမ်းမရှိပါ</div>;
      }
  };
  ...
  const getStats = (period) => {
    ...
    const isSameDate = (dbDateStr, dNum, mNum, yNum) => {
        if (!dbDateStr) return false;
        const parts = dbDateStr.split('/');
        if (parts.length !== 3) return false;
        return parseInt(parts[0], 10) === dNum && 
               parseInt(parts[1], 10) === parseInt(mNum, 10) && 
               parseInt(parts[2], 10) === parseInt(yNum, 10);
    };

    const todayParts = todayStr.split('/');
    const todayD = parseInt(todayParts[0], 10);
    const todayM = parseInt(todayParts[1], 10);
    const todayY = parseInt(todayParts[2], 10);

    const activeDaysCache = {};
    const isOfficeActiveDay = (dNum, mNum, yNum) => {
        const cacheKey = `${dNum}-${mNum}-${yNum}`;
        if (activeDaysCache[cacheKey] !== undefined) return activeDaysCache[cacheKey];
        const isActive = records.some(r => isSameDate(r.date, dNum, mNum, yNum) && !r.isAutoGenerated && !r.isAbsent);
        activeDaysCache[cacheKey] = isActive;
        return isActive;
    };
    ...
    return employees.map(emp => {
      ...
      if (period === 'today') {
        ...
        // We will keep all records for this employee without filtering early,
        // because we will match them dynamically inside the loop using isSameDate.
      }

      const totalLate = empRecords.reduce((sum, r) => {
        if (!r.date) return sum;
        let dNum = parseInt(r.date.split('/')[0], 10);
        if (period === 'month' && !isSameDate(r.date, dNum, sMonth, sYear)) return sum; // Filter manually here for month total late
      ...
      if (period === 'today') {
         const hasCheckedInToday = empRecords.some(r => r.date === todayStr);
         const hasAbsentRecordToday = empRecords.some(r => r.date === todayStr && r.isAbsent);
         
         if (hasAbsentRecordToday && !isTodayOffDay) {
             absences = 1;
         } else if (!hasCheckedInToday && isPastNoon && !isTodayOffDay) {
             absences = 1;
         }
      } else if (period === 'month') {
         const yearNum = parseInt(sYear, 10);
         const monthNum = parseInt(sMonth, 10) - 1;
         const lastDayOfMonthDate = new Date(yearNum, monthNum + 1, 0);
         const lastDayOfMonth = lastDayOfMonthDate.getDate();
         
         let limitDay = lastDayOfMonth;
         if (isCurrentMonthView) {
             limitDay = todayDateNum;
         } else if (yearNum > now.getFullYear() || (yearNum === now.getFullYear() && monthNum > now.getMonth())) {
             limitDay = 0;
         }
         ...
         for (let d = 1; d <= limitDay; d++) {
             ...
             if (dayRecords.length > 0) {
                 if (dayRecords.some(r => r.isAbsent)) {
                     absences += 1;
                 }
             } else {
                 if (isCurrentMonthView && d === limitDay) {
                     if (isPastNoon && officeActive) {
                         absences += 1;
                     }
                 } else {
                     if (officeActive) {
                         let skipBecauseNotHired = false;
                         if (emp.createdAt) {
                             const empCreatedAtStartOfDay = new Date(emp.createdAt);
                             if (!isNaN(empCreatedAtStartOfDay.getTime())) {
                                 empCreatedAtStartOfDay.setHours(0,0,0,0);
                                 // No backward leniency. Only track from actual creation date forward.
                                 const checkDate = new Date(yearNum, monthNum, d);
                                 if (checkDate < empCreatedAtStartOfDay) {
                                     skipBecauseNotHired = true;
                                 }
                             }
                         }
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 no-print">
              ...
              <div className="space-y-3">
                {employees.map(emp => {
                    const empRegRecs = records.filter(r => {
                        if (!r.date || r.employeeId !== emp.id) return false;
                        const parts = r.date.split('/');
                        if (parts.length !== 3) return false;
                        const now = new Date();
                        return parseInt(parts[0], 10) === now.getDate() && 
                               parseInt(parts[1], 10) === now.getMonth() + 1 && 
                               parseInt(parts[2], 10) === now.getFullYear();
                    });
                    
                    const tStat = todayStats.find(e => e.id === emp.id);
                    
                    // If employee has actual records today, map them
                    if (empRegRecs.length > 0) {
                        return empRegRecs.map(rec => (
                    ...
                    // If employee has NO actual record, but todayStats says they are absent, show an auto-generated row
                    if (tStat && tStat.absences > 0) {
                        return (
                           <div key={'auto-'+emp.id} className="flex items-center gap-4 p-3 rounded-xl border border-slate-100 bg-white">
                              <img src={`https://ui-avatars.com/api/?name=${encodeURIComponent(emp.name)}&background=f43f5e&color=fff`} alt="auto" className="w-12 h-12 rounded-full object-cover border-2 border-slate-200" />
                              <div className="flex-1">
                                <h4 className="font-semibold text-sm">{emp.name}</h4>
                                <p className="text-xs text-slate-500">{new Date().toLocaleDateString('en-GB')} • 12:00 PM</p>
                              </div>
                              <div className="text-right">
                                <span className="inline-flex items-center gap-1 text-xs font-medium text-red-600 bg-red-50 px-2 py-1 rounded-full border border-red-100"><XCircle className="w-3 h-3" /> ပျက်ရက်</span>
                              </div>
                ...
                {(!records.some(r => {
                      ...
                      return parseInt(parts[0], 10) === now.getDate() && 
                             parseInt(parts[1], 10) === now.getMonth() + 1 && 
                             parseInt(parts[2], 10) === now.getFullYear();
                }) && !todayStats.some(s => s.absences > 0)) && (
      ...
      {detailModalEmp && (
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl w-full max-w-sm p-6 shadow-2xl animate-in zoom-in duration-200">
             <div className="flex justify-between items-center mb-4">
                <h3 className="text-xl font-bold text-slate-800 flex items-center gap-2"><ClipboardList className="w-5 h-5 text-indigo-500" /> အသေးစိတ် အခြေအနေ</h3>
                <button onClick={() => setDetailModalEmp(null)} className="text-slate-400 hover:text-slate-600"><XCircle className="w-6 h-6" /></button>
             </div>
             <p className="text-sm font-bold text-slate-700 mb-4">{detailModalEmp.name} <span className="text-slate-500 font-medium">({detailModalEmp.position})</span></p>

             <div className="space-y-4">
                <div>
                   <label className="block text-sm font-semibold text-slate-700 mb-1">ရက်စွဲ ရွေးချယ်ရန်</label>
                   <input type="date" value={detailModalDate} onChange={e => setDetailModalDate(e.target.value)} className="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 font-bold" />
                </div>
                
                <div className="p-6 bg-slate-50 border border-slate-200 rounded-xl flex justify-center items-center min-h-[120px]">
                   {renderDetailModalStatus()}
                </div>
             </div>
          </div>
        </div>
